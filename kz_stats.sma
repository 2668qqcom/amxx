...
new Float:rDistance[3],Float:frame2time,bool:touch_ent[33],bool:ddbeforwj[33],bool:gHasColorChat[33],bool:chatinfo[33],Float:old_angle1[33];
...
public plugin_init()
{
	...
	register_dictionary("uq_jumpstats2.51.txt");
	...
	//botstats
	register_clcmd( "say /botstats",	"cmdbotstats",         ADMIN_ALL);
...
public plugin_precache()
{
	...
	precache_sound( "misc/impressive.wav" );//1
	precache_sound( "misc/perfect.wav" );//2
	precache_sound( "misc/holyshit.wav" );//3
	//precache_sound( "misc/mod_godlike.wav" );//4
	precache_sound( "misc/mod_wickedsick.wav" );//4
	precache_sound( "misc/monsterkill.wav" );//5
...
//botstats
public cmdbotstats(id)
{
	g_botstats[id] = !g_botstats[id];
	if(g_botstats[id])
	{
		ColorChat(id,BLUE,"^1[%s] Botstats: ^3enabled", prefix);
	}
	else 
	{
		ColorChat(id,RED,"^1[%s] Botstats: ^3disabled", prefix);
	}
				...
				{	
					...
					if(strafe_num[id] < NSTRAFES)
					{
						...
						for(jj = 1;jj <= strafe_num[id]; jj++)
						{
							...
							new Float:tmp_gain = strafe_stat_speed[id][jj][0];
							new Float:tmp_loss = strafe_stat_speed[id][jj][1];
							new Float:tmp_time = time_;
							new tmp_sync = sync_[id];


							new gain_str[16];
							if(tmp_gain < 10.0) {
							    format(gain_str, 15, "^t^t^t^t%0.1f", tmp_gain);
							} else if(10.0 <= tmp_gain < 100.0) {
							    format(gain_str, 15, "^t^t^t%0.1f", tmp_gain);
							} else {
							    format(gain_str, 15, "^t^t %0.1f", tmp_gain);  
							}


							new loss_str[16];
							if(tmp_loss < 10.0) {
							    format(loss_str, 15, "^t^t^t^t%0.1f", tmp_loss);
							} else if(10.0 <= tmp_loss < 100.0) {
    							format(loss_str, 15, "^t^t^t%0.1f", tmp_loss);
							} else {
							    format(loss_str, 15, "^t^t %0.1f", tmp_loss);
							}

						
							new time_str[16];
							if(tmp_time < 10.0) {
							    format(time_str, 15, "^t^t^t^t%0.0f", tmp_time);
							} else if(10.0 <= tmp_time < 100.0) {
							    format(time_str, 15, "^t^t^t%0.0f", tmp_time);
							} else {
							    format(time_str, 15, "^t^t %0.0f", tmp_time);
							}

						
							new sync_str[16];
							if(tmp_sync < 10) {
 							   format(sync_str, 15, "^t^t^t %3d", tmp_sync);
							} else if(10.0 <= tmp_sync < 100) {
 							   format(sync_str, 15, "^t^t^t%3d", tmp_sync);
							} else {
							    format(sync_str, 15, "^t^t %3d", tmp_sync);
							}
							strLen += format(strMess[strLen], (40*NSTRAFES)-strLen-1, "%L", LANG_SERVER, "UQSTATS_HUDSTRSTATS1", 
 							               jj, gain_str, loss_str, time_str, sync_str);
						...
						if(strafe_num[id]!=0)
						{
							...
							{
								strLen =strLen+format(strMess[strLen],(40*NSTRAFES)-strLen-1, "%L",LANG_SERVER,"UQSTATS_HUDSTRSTATS9",edgedist[id],float(jumpblock[id]) - distance[id] + edgedist[id],jumpblock[id]);
							}
					...
					for( new i = INFO_ONE; i < max_players; i++ )
					{
						...
						{	
							...
							if(multibhoppre[i]==true && jump_type[id]==Type_Multi_Bhop &&!failed_jump[id])
							{
								...
								show_hudmessage( i, "%L",LANG_SERVER,"UQSTATS_HUD_PRINT1_1", type, distance[id], maxspeed[id], gain[id], prestrafe[id],strafe_num[id], sync_[id]);
							}
							...
							else if((jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_Drop_BhopLongJump || jump_type[id]==Type_WeirdLongJump || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump) &&!failed_jump[id])
							{
								set_hudmessage(stats_r, stats_g, stats_b, stats_x, stats_y, 0, 6.0, 2.5, 0.1, 0.3, h_stats  );
								if(jump_type[id]==Type_HighJump){
									new Float:edge_diff = (13.0 - edgedist[id] >= 0.0) ? (13.0 - edgedist[id]) : 0.0;
									show_hudmessage(i, "%L", LANG_SERVER, "UQSTATS_HUD_PRINT_FF0", type, distance[id], edge_diff, maxspeed[id], gain[id], prestrafe[id], strafe_num[id], sync_[id]);						
								}else{
						...
						if( (i == id || is_spec_user[i]) && g_lj_stats[i]==true)
						{
							...
							if(Show_edge[id] && failed_jump[id]==false && jump_type[id]!=Type_ladder && jumpblock[id]<user_block[id][0] && jumpblock[id]>user_block[id][1] && edgedist[id]<100.0 && edgedist[id]!=0.0 && (jumpblock[id]+edgedist[id])<distance[id])
							{
								if((jumpblock[id]+Landing+edgedist[id])>(distance[id]+10.0) || Landing<=0.0)
								{
									...
									client_print( i, print_console,"%L",LANG_SERVER,"UQSTATS_CONSOLE_PRINT20",jumpblock[id],edgedist[id],Landing); //botstats?
							}
					...
					{ 
						...
						{	
						//botstats
							if((!failed_jump[id] && !is_user_bot(id)) || (!failed_jump[id] && g_botstats[ids]))
							{
											...
											|| jump_type[id]==Type_ladderBhop || jump_type[id]==Type_ladder || jump_type[id]==Type_LongJump || jump_type[id]==Type_HighJump))
								{
									if ( distance[id] >= god_dist ) {
										if( uq_sounds && enable_sound[ids]==true )
										{
											...
											client_cmd(ids, "speak misc/monsterkill");
											else
										...
										for(new i = INFO_ONE; i < max_players; i++ )
										{
											if( (i == id || is_spec_user[i]))
											{
												//if( uq_sounds && enable_sound[i]==true ) client_cmd(i, "speak misc/mod_godlike");
